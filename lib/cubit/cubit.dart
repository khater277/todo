...
class TodoCubit extends Cubit<TodoStates> {
  ...
  void changeAppLanguage(String value) {
    lang = value;
    GetStorage().write('lang', value).then((v) {
  ...
  void setTaskTime(DateTime dateTime) {
    ...
    print("============>$currentIndex");
    emit(TodoSetTaskTimeState(index: currentIndex));
  }

  void setWhenColor(BuildContext context, int index) async {
    if (index == 2) {
      if (taskDateTime != null) {
        currentIndex = index;
        final date = await context.datePicker(initialDate: DateTime.now());
        if (date != null) {
    ...
    emit(TodoSetWhenColorState(index: currentIndex));
  }
  ...
  void taskOperation({
    ...
    if (taskDate.year == yearDate && taskDate.month == monthDate) {
      if (isAdd == true) {
        allTasks[2].add(task);
      } else {
  ...
  void addNewTask({
    ...
    Future.delayed(Duration(
            seconds: dateTime.difference(DateTime.now()).inSeconds + 1))
        .then((value) {
  ...
  void deleteTask({required int index, required TaskModel task}) async {
    ...
    await FlutterLocalNotificationsPlugin()
        .cancel((task.dateTime!.millisecondsSinceEpoch / 1000).floor());
    getAllTasks();
